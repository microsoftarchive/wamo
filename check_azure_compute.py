...
def handle_args():
    parser = argparse.ArgumentParser(description='Check Azure.',
...
def check_for_errors(service):
    ...
    for service in hosted_services:
        hosted_service = service.get_hosted_service_properties(
            service.service_name, embed_detail=True)
        for deployment in hosted_service.deployments:
            if deployment.status != 'Running':
                errors.append(
                    'Hosted Service {}, Deployment {}, Status: {}'.format(
                        hosted_service.service_name, deployment.name,
                        deployment.status))
            for role_inst in deployment.role_instance_list:
                if role_inst.power_state != 'Started':
                    errors.append(
                        'Hosted Service {}, Deployment {}, Role {}, Power: {}'.format(
                            hosted_service.service_name, deployment.name,
                            role_inst.role_name, role_inst.power_state))
                if role_inst.instance_status != 'ReadyRole':
                    errors.append(
                        'Hosted Service {}, Deployment {}, Role {}, Status: {}'.format(
                            hosted_service.service_name, deployment.name,
                            role_inst.role_name, role_inst.instance_status))
    return errors
